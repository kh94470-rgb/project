...
-- SQL문장 내에서 변수 정의, 조건처리, 반복처리 등 지원하여 SQL 단점 보완
...
DECLARE
    EMP_ID EMPLOYEE.EMP_ID%TYPE; -- 레퍼런스 변수, 어떤 타입인지 갖고 옴
...
BEGIN
    ...
    WHERE EMP_ID = '&EMP_ID';
        ...
        DBMS_OUTPUT.PUT_LINE('NATIONAL_CODE : ' || TEAM);
END;
/

-- 사용자에게 사번을 받아와 그 사원이 전체 정보를 VEMP에 저장 후
...
BEGIN 
    ...
    LOOP
        ...
        VALUES(SEQ_TEST.NEXTVAL, '100'); --쓰레기 데이터 많이 쓰기
    END LOOP;
END;
/
